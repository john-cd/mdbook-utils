...
"#,
    ...
    let unused = identify_unused_rs_examples(&md_dir, &code_dir)?;
    ...
    let unused = identify_unused_rs_examples(&md_dir, &code_dir)?;
    let secret_canon = secret_file.canonicalize()?;
...
"#,
    ...
    assert!(!content.contains("SENSITIVE DATA"), "Vulnerability still present: Included file from outside base directory!");
...
"#,
    ...
    let missing = identify_files_not_in_summary(&md_dir)?;
